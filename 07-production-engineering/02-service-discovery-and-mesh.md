...
A few figures anchor the decision. The registry itself is a coordination service and runs on a **fixed small cluster of 3 or 5 nodes** regardless of how many services it tracks — 3 tolerates one failure, 5 tolerates two <abbr title="[pp. 439–440]">[i]</abbr>. Do not scale it with your fleet; its data (endpoints, leadership) changes over seconds-to-minutes, not milliseconds <abbr title="[p. 439]">[i]</abbr>. Registry *reads*, being cache-friendly and non-linearizable, can be fanned out cheaply to observers or local caches <abbr title="[p. 440]">[i]</abbr>; registry *writes* and any linearizable read pay a quorum round-trip, which is expensive across regions <abbr title="[p. 442]">[i]</abbr> — a reason to keep the registry regional and the write rate low.
