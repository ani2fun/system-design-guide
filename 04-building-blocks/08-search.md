...
1. **Tokenize.** Split the raw text into candidate words. `"Great sound, terrible battery!"` becomes `["Great", "sound", "terrible", "battery"]` — punctuation dropped, split on whitespace and boundaries. Tokenization is language-specific: splitting English on spaces is easy; Chinese has no spaces, and German glues compound nouns together <abbr title="[p. 146]">[i]</abbr>.
...
Immutability is the same bargain the LSM storage engine makes, and the consequences follow directly <abbr title="[p. 147]">[i]</abbr>:
