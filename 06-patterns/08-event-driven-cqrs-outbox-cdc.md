...
A runnable implementation lives at `_proof-of-concepts/06-patterns/08-event-driven-cqrs-outbox-cdc/` in the repo root — a real Postgres (via `./run`), a Unit of Work that writes the order and its event in one transaction, a relay that polls the outbox with `FOR UPDATE SKIP LOCKED`, and an idempotent consumer.

```bash
cd _proof-of-concepts/06-patterns/08-event-driven-cqrs-outbox-cdc
...
**Run it yourself.** [Transactional outbox](https://github.com/ani2fun/system-design-guide/tree/main/_proof-of-concepts/06-patterns/08-event-driven-cqrs-outbox-cdc)
...
`_proof-of-concepts/06-patterns/08-event-driven-cqrs-outbox-cdc/`, run `./run`.
