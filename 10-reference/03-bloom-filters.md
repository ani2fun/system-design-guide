...
**The decision is rarely "Bloom filter or nothing."** It is a choice among several ways to answer *"is this thing probably in the set?"*, and the right one depends on whether you need deletion, whether you know the set size up front, and how expensive the lookup you are trying to avoid actually is.

| Option | Gives you | Costs you | Use when |
| --- | --- | --- | --- |
| **Bloom filter** | ~10 bits/element at 1% error; no false negatives; constant-time add and test | No deletion, no resize, no enumeration; `n` must be known to size it | The set is append-only and roughly sized in advance — the common case |
| **Counting Bloom filter** | Deletion, by replacing bits with small counters | ~4× the space; counters can saturate under skew | Elements genuinely leave the set and space is not the binding constraint |
| **Cuckoo filter** | Deletion natively, and better space efficiency below ~3% error | More complex; inserts can fail once the table is loaded | You need deletion *and* a tight error rate |
| **Scalable Bloom filter** | Bounded error as the set grows past its original estimate | Layered lookups get slower as chains lengthen | `n` is genuinely unknown up front |
| **Exact hash set** | A definite answer — no false positives at all | Stores the elements themselves, so space grows with element size | The set fits comfortably in memory, or a wrong "maybe" is expensive |
| **No filter at all** | Zero complexity | Every lookup pays full cost | The lookup you would skip is already cheap — see the pitfall below |

<div style="border-left:4px solid #15448e;background:rgba(21,68,142,0.08);padding:0.6rem 1rem;border-radius:0 0.5rem 0.5rem 0;margin:1.25rem 0">

📘 **The economics in one line.** A Bloom filter pays off when *(miss rate) × (cost of the avoided lookup)* exceeds the cost of consulting the filter. Against a local in-memory map the trade rarely pays; against a disk seek or a network round trip it pays enormously — which is exactly why LSM-tree engines put one in front of every SSTable.
