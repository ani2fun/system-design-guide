...
To store a key, append `key,value` to a text file. To read a key, `grep` the file and take the *last* line that matches, because the newest write wins (p. 115). It works. It is genuinely durable — the data is on disk — and writes are as fast as your disk can append.
...
- **Page-oriented:** The other says: *keep data in a fixed, navigable structure on disk and overwrite it carefully in place.* That is the **page-oriented** approach — the **B-tree** — and it is the standard index in almost every relational database, PostgreSQL and MySQL's InnoDB included (pp. 121, 125–126).
...
- Pages reference each other by page number, which acts as a disk pointer: page number × page size gives the byte offset. This builds a tree of pages (p. 126).
- Every lookup starts at the single **root page**, whose entries carry key ranges and child pointers. You pick the child whose range contains your key, follow it down, and repeat until you reach a **leaf page**, which holds the value inline or a reference to where the value is stored (p. 126).
- **Branching factor:** The number of child references packed into one page — the **branching factor** — is typically several hundred (p. 126).
- A high branching factor means a shallow tree: because depth is `O(log n)`, most databases fit in a three- or four-level tree, and a four-level tree of 4 KiB pages with branching factor 500 can address up to **250 TB** (p. 127).
...
- If it's full, the page must **split** into two half-full pages, and the parent page is updated to reference both (Fig. 4-6, pp. 126–127).
...
- First, the B-tree writes every piece of data **at least twice** — once to the WAL, once to the tree page — and often writes a whole page even for a few changed bytes (pp. 130–131).
- Second, not every B-tree engine uses a WAL: LMDB instead uses *copy-on-write*, writing a modified page to a new location and pointing a fresh parent version at it, which doubles as a clean mechanism for concurrency and snapshots (p. 128).
...
**Write amplification** is defined precisely as *bytes actually written to disk ÷ bytes a plain append-only log would have written* (p. 131).
...
- For typical workloads LSM-trees have *lower* write amplification overall, because they avoid full-page writes and compress their SSTables — and lower write amplification means more useful writes per second within a fixed disk bandwidth (p. 131).

**Sequential versus random writes** is why this matters on real hardware.

- B-trees scatter small random page writes across the disk; LSM engines write whole segment files (far larger than a page) sequentially (pp. 129–130).
- Disks strongly favor sequential throughput — dramatically so for HDDs, but for SSDs too: flash reads and writes a ~4 KiB page but *erases* a ~512 KiB block, so random writes force more garbage collection inside the drive and wear it out faster than sequential writes (p. 130).
...
- B-trees fragment as pages are freed mid-file, stranding space and needing a background vacuum (PostgreSQL's `VACUUM` is exactly this) (p. 131).
...
**RocksDB and LevelDB** are the canonical embeddable LSM engines — libraries you link into your process rather than a server you talk to over a socket (p. 125).
